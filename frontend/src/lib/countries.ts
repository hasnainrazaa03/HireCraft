 ...
 * alpha-2 code as regional-indicator emoji at render time (no image assets).
 * Ordered with the most common origins first, then alphabetical.
 */
...
// Priority countries surfaced at the top of the list.
const PRIORITY = ["US", "IN", "GB", "CA", "AU", "DE"];
...
const rest = ALL.filter((c) => !PRIORITY.includes(c.iso)).sort((a, b) =>
  a.name.localeCompare(b.name),
);
export const COUNTRIES: Country[] = [
  ...PRIORITY.map((iso) => ALL.find((c) => c.iso === iso)!),
  ...rest,
];
