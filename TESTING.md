...
| **Workday** | **Run against a live form (Applied Materials, *My Information*); one round of fixes in, re-run pending.** 634 postings across 218 tenants, all under `*.myworkdayjobs.com` — one match pattern reaches every one, and `data-automation-id` names the controls. First run filled First name, Last name, City and Phone Number, and left Country and Phone Device Type alone as already answered. What it found is that **Workday builds every dropdown as a `<button>`, not as a text input** — a different shape from Greenhouse/Ashby/Lever rather than a variation on it: nothing to type into on the element itself (the search box is a separate unlabelled input beside it, which the scan had been discarding), a `value` that is a database key (`bc33aa3152ec42d4995f4791a106ed09`), a prompt of "Select One" with no ellipsis to mark it as one, and a popup that is never linked back by `aria-controls`. Each was an assumption in the engine; the first threw `Illegal invocation` and killed the State field. All four are handled now. **Also: most tenants require an account before the form appears, and creating one is a human step the extension will not take.** |
| Workday — left for you by design | *How Did You Hear About Us?* (nothing stored), *Address Line 1* / *Postal Code* (not in Career Profile), and *"Have you ever worked at &lt;company&gt;…"* — a rehire question, required on nearly every Workday form. An application is signed, so guessing "No" would be a false statement about someone's employment history rather than a blank field. All three are reported rather than passed over. |
