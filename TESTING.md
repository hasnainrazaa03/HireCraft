...
- ✅ **T-RES-01** **New / Import** → upload `~/Downloads/MHR_ML.pdf`. Parsing runs (~a few seconds, one LLM call) → lands in the **builder** pre-filled.
  - Notes: PASS (2026-08-23, `MHR_AIML.tex`). Import → parse → builder, pre-filled. Template applied is the new **`classic`** default, so the résumé keeps its own layout instead of being restyled. Original `.tex` stored (9,122 bytes) and now **readable in-app** via the *Original* button, not download-only (commit b6e8716).
...
- ✅ **T-RES-03** Parsed structure is complete: 3 experience, 2 education, 2 projects, skills groups; bullets not clipped.
  - Notes: PASS — verified against the source `.tex`, not by eye: **3 experience · 2 education · 2 projects · 6 skill groups · 15 bullets**, matching the file exactly. **Project Vimaan present** (the P0 that was silently deleted last run — the denser `classic` layout now fits all 15 bullets on one page with the trimmer untouched). **No invented summary or headline** — both empty, as the source has no summary section. Two defects found by rendering and fixed (commit b70a031): (a) the degree line read *"M.S. in Computer Science in Computer Science"* — the template appended `field_of_study` even when the degree already named it; (b) **"Silver Medalist" was silently dropped** from `GPA: 3.86, Silver Medalist` — fixed in the parser prompt *and* by a raw-payload repair that splits grade from distinction before validation (`gpa` caps at 20 chars, so a combined value would be rejected and the honour lost entirely). Re-import confirmed both.
...
- ⬜ **T-RES-17** **One-page fit is the default** (resolves backlog #14): a new/edited résumé fits a **single page** by default — the renderer auto-tightens spacing (escalating a compaction density to a readable floor) across **preview, exports, and tailored versions**. The editor checkbox is now **"Allow more than one page"** (checked = let it run onto a second page), default **off**. Existing résumés were migrated on. *(Verified: the real master résumé compacts 2 pages → 1 under one-page fit.)*
  - Notes:
- ⬜ **T-RES-18** **Tailoring is technical + quantified** (new emphasis): a tailored résumé keeps the **specific technical substance** — real methods/architectures/algorithms/libraries (e.g. "depthwise-separable 3D CNNs", "INT8 quantization", "REST orchestration across Pega/MDM/SAP"), never flattened to vague "deep learning models"/"backend work" — and **leads with real metrics** wherever the master résumé/brag bank has them. Still **zero fabrication**: the guardrail deletes any invented number/tool.
  - Notes:
...
| **Workday** | **Verified end to end on a live form**, across both steps. Applied Materials: *My Information* (name, address, city, state, phone, country) and *My Experience* (two education blocks with degrees, three work-experience blocks with dates and role descriptions, two websites, the résumé). `missing` and `required` both came back empty. 634 postings across 218 tenants, all under `*.myworkdayjobs.com` — one match pattern reaches every one, and `data-automation-id` names the controls. Getting there took seven rounds against the live page, because **Workday's controls are a different shape from Greenhouse/Ashby/Lever rather than a variation on them**: every dropdown is a `<button>` with its search box beside it and a database key for a value; dates are a `div` per segment wrapping a two-character input that wants `05` and not `May`; a section starts empty behind an `Add` button that says only "Add", then moves inside the block and says "Add Another"; and the uploader clears its own input after accepting a file. **Also: most tenants require an account before the form appears, and creating one is a human step the extension will not take.** |
| **Workday — Skills** | **The one thing on the page that does not fill.** `input#skills--skills` is a search-backed multi-select; typing into it produces no suggestions, and nothing found opens its menu — not the keyboard, not a click on the control, and the prompt icon beside it is never located from the input at all (`openedBy: "nothing"`). Four rounds of theories, each disproved by the next dump: it was not the option markup, not assigning instead of typing, and not the open/close churn — though all three were real problems worth fixing on their own. It is optional on every form seen so far, never appearing in `required`, and the fill now gives up after three silent queries rather than spending twenty seconds proving the same point. Add skills by hand. |
