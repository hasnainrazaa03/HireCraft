...
| **Workday** | **First pass, unverified.** 634 postings across 218 tenants, all under `*.myworkdayjobs.com` — one match pattern reaches every one. Its controls carry `data-automation-id`, which is semantic and stable, and those names map onto the catalogue already (`legalNameSection_firstName`, `phone-number`, `addressSection_city`). Not yet run against a real page: the shell ships zero inputs, so the form cannot be read from outside a browser, exactly as with Ashby. **Also: most tenants require an account before the form appears, and creating one is a human step the extension will not take.** |
